...
// Импортируем компонент BoardSettings
import BoardSettings from './features/Settings/BoardSettings'; 

// --- ТИПЫ ДАННЫХ ---
export interface ChecklistItem { id: string; text: string; completed: boolean; children: ChecklistItem[]; }
export interface Status { _id: string; name: string; }
export interface Column { _id: string; name: string; statusIds: string[]; }
export interface Project { _id: string; name: string; columns: Column[]; statuses: Status[]; }
export interface User { _id: string; login: string; }
export interface Task { _id: string; title: string; description: string; status: string; assignee?: User; createdAt: string; priority: number; activityLog: ActivityLog[]; checklist: ChecklistItem[]; } // <-- ДОБАВЛЕНО ПОЛЕ checklist
export interface ActivityLog { user: { login: string }; action: string; field?: string; oldValue?: string; newValue?: string; timestamp: string; }
...
const GlobalStyles = () => (
    <style>{`
        ...
        .settings-section { background-color: var(--light-bg); padding: 20px; border-radius: var(--border-radius); }
        .settings-list { min-height: 200px; border: 1px dashed #ccc; border-radius: 4px; padding: 10px; background-color: white; flex-grow: 1; }
        .settings-item { background-color: #ffffff; padding: 10px; border-radius: 4px; margin-bottom: 8px; border: 1px solid var(--border-color); display: flex; justify-content: space-between; align-items: center; cursor: grab; touch-action: none; }
        .settings-item span { flex-grow: 1; padding: 5px; }
        .add-item-form { display: flex; gap: 10px; }
        ...
        .member-list { list-style: none; padding: 0; max-height: 150px; overflow-y: auto; background-color: white; border: 1px solid var(--border-color); border-radius: 4px; padding: 5px; }
        .member-item { padding: 8px 12px; border-bottom: 1px solid #eee; }
...
// --- НОВЫЙ КОМПОНЕНТ ДЛЯ ЧЕК-ЛИСТА ---
const findAndToggle = (items: ChecklistItem[], id: string): ChecklistItem[] => items.map(item => ({ ...item, completed: item.id === id ? !item.completed : item.completed, children: findAndToggle(item.children, id) }));
const findAndAddChild = (items: ChecklistItem[], parentId: string, newText: string): ChecklistItem[] => items.map(item => item.id === parentId ? { ...item, children: [...item.children, { id: uuidv4(), text: newText, completed: false, children: [] }] } : { ...item, children: findAndAddChild(item.children, parentId, newText) });
const ChecklistItemView = ({ item, level = 0, onToggle, onAddChild }: { item: ChecklistItem; level?: number; onToggle: (id: string) => void; onAddChild: (parentId: string) => void; }) => (
    <div style={{ marginLeft: `${level * 25}px`, marginTop: '5px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            {level < 2 && (<button onClick={() => onAddChild(item.id)} title="Добавить подпункт" style={{ padding: '2px 6px' }}>+</button>)}
        </div>
        {item.children.map(child => <ChecklistItemView key={child.id} item={child} level={level + 1} onToggle={onToggle} onAddChild={onAddChild} />)}
    </div>
);
function TaskChecklist({ initialChecklist, onChecklistChange }: { initialChecklist: ChecklistItem[], onChecklistChange: (newChecklist: ChecklistItem[]) => void }) {
    const [checklist, setChecklist] = useState(initialChecklist);
    const [newItemText, setNewItemText] = useState('');

    useEffect(() => { onChecklistChange(checklist); }, [checklist, onChecklistChange]);
    ...
    const handleAddChild = (parentId: string) => { const text = prompt("Введите текст нового подпункта:"); if (text && text.trim()) { setChecklist(current => findAndAddChild(current, parentId, text.trim())); } };
    const handleAddNewRootItem = () => { if (!newItemText.trim()) return; setChecklist([...checklist, { id: uuidv4(), text: newItemText.trim(), completed: false, children: [] }]); setNewItemText(''); };

    return (
        <div>
            <h4 style={{ marginTop: '20px', borderBottom: '1px solid #eee', paddingBottom: '10px' }}>Чек-лист</h4>
            {checklist.map(item => <ChecklistItemView key={item.id} item={item} onToggle={handleToggle} onAddChild={handleAddChild} />)}
...
function TaskDetailModal({ task, members, statuses, onClose, onUpdate, onDelete, token }: { task: Task | null, members: User[], statuses: Status[], onClose: () => void, onUpdate: (updatedTask: Task) => void, onDelete: (taskId: string) => void, token: string }) { 
    ...
    // Новая функция для обновления состояния чек-листа
    ...
    const handleSave = async () => { 
        ...
        try { 
            const response = await axios.patch<Task>(`/api/tasks/${editableTask._id}`, { 
                ...
                checklist: editableTask.checklist // <-- Отправляем чек-лист на сервер
