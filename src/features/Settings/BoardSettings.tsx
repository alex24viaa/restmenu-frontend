// src/features/Settings/BoardSettings.tsx
...
// Импортируем типы из App.tsx, предполагая, что они там определены.
// Если вы вынесете типы в отдельный файл (например, types.ts), то измените этот путь.
import type { Project, Column, Status, User } from '../../App'; 

// --- ИКОНКИ (если они используются только здесь, или импортируйте их) ---
...
export default function BoardSettings({ project, token, onSettingsChange, onDirtyChange }: { project: Project, token: string, onSettingsChange: (updatedProject: Project) => void, onDirtyChange: (isDirty: boolean) => void }) {
    // Явно типизируем useState для localColumns и localStatuses
    ...
    const handleDragEnd = (event: DragEndEvent) => {
        ...
        setLocalColumns((prevCols: Column[]) => { // Явно типизируем prevCols
            const newCols = prevCols.map((c: Column) => ({ // Явно типизируем c
                ...c, 
                statusIds: c.statusIds.filter((id: string) => id !== activeId) // Явно типизируем id
            }));

            if (overContainerType === 'column') {
                const overColIndex = newCols.findIndex((c: Column) => c._id === overContainerId); // Явно типизируем c
    ...
    const handleAddNewStatus = () => { 
        ...
        setLocalStatuses((prev: Status[]) => [...prev, newStatus]); // Явно типизируем prev
        setNewStatusName(''); 
    };
    ...
    const handleAddNewColumn = () => { 
        ...
        setLocalColumns((prev: Column[]) => [...prev, newColumn]); // Явно типизируем prev
        setNewColumnName(''); 
    };

    const handleDeleteStatus = (statusId: string) => { 
        if (window.confirm('Удалить статус?')) { 
            setLocalStatuses((prev: Status[]) => prev.filter((s: Status) => s._id !== statusId)); // Явно типизируем prev и s
            setLocalColumns((prev: Column[]) => prev.map((c: Column) => ({...c, statusIds: c.statusIds.filter((id: string) => id !== statusId)}))); // Явно типизируем prev, c и id
        } 
    };

    const handleRemoveStatusFromColumn = (colId: string, statusId: string) => { 
        setLocalColumns((prev: Column[]) => prev.map((c: Column) => c._id === colId ? { ...c, statusIds: c.statusIds.filter((id: string) => id !== statusId) } : c)); // Явно типизируем prev, c и id
    };

    const handleDeleteColumn = (colId: string) => { 
        if (window.confirm('Удалить колонку?')) { 
            setLocalColumns((prev: Column[]) => prev.filter((c: Column) => c._id !== colId)); // Явно типизируем prev и c
        } 
    };

    const handleUpdateStatus = (statusId: string, newName: string) => { 
        setLocalStatuses((prev: Status[]) => prev.map((s: Status) => s._id === statusId ? { ...s, name: newName } : s)); // Явно типизируем prev и s
    };

    const handleUpdateColumn = (colId: string, newName: string) => { 
        setLocalColumns((prev: Column[]) => prev.map((c: Column) => c._id === colId ? { ...c, name: newName } : c)); // Явно типизируем prev и c
    };

    const unassignedStatuses = useMemo(() => { 
        const assignedStatusIds = new Set(localColumns.flatMap((c: Column) => c.statusIds)); // Явно типизируем c
        return localStatuses.filter((s: Status) => !assignedStatusIds.has(s._id)); // Явно типизируем s
                    ...
                    <div className="kanban-board" style={{ padding: '10px 0', borderTop: '1px solid var(--border-color)' }}>
                        <div className="kanban-column" style={{width: '260px'}}>
                            ...
                            <div ref={setStatusPoolRef} className="settings-list">
                                <SortableContext items={unassignedStatuses.map((s: Status) => s._id)}> {/* Явно типизируем s */}
                                    {unassignedStatuses.map((status: Status) => <SortableStatus key={status._id} status={status} onUpdate={(newName) => handleUpdateStatus(status._id, newName)} onDelete={() => handleDeleteStatus(status._id)} />)} {/* Явно типизируем status */}
                                </SortableContext>
                            </div>
                        </div>
                        {localColumns.map((column: Column) => <DroppableColumn key={column._id} id={column._id} name={column.name} statusIds={column.statusIds} statuses={localStatuses} onUpdateColumn={handleUpdateColumn} onRemoveStatus={handleRemoveStatusFromColumn} onRemoveColumn={handleDeleteColumn} />)} {/* Явно типизируем column */}
                        <div className="kanban-column" style={{width: '260px'}}>
